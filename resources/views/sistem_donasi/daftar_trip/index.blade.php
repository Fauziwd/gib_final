    ...
    <div class="container mx-auto sm:px-4 p-6 rounded h-screen">
        <div class="flex flex-wrap">
            <div class="lg:w-full pr-4 pl-4 margin-tb">
                ...
                <div class="mt-10 mb-6">
                    <a href="{{ route('daftar_trip.create') }}" class="text-white bg-gradient-to-r from-teal-400 via-teal-500 to-teal-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-teal-300 dark:focus:ring-teal-800 shadow-lg shadow-teal-500/50 dark:shadow-lg dark:shadow-teal-800/80 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2">Tambah Daftar Trip</a>
                </div>
            </div>
        </div>
        ...
        <div class="relative overflow-x-auto shadow-2xl sm:rounded-lg">
            ...
            <tr class="bg-white border-b mb-4 dark:bg-gray-800 dark:border-gray-700">
                    ...
                    <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $daftarTrip->nama_trip }}</td>
                    <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $daftarTrip->keterangan }}</td>
                    <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                        <form action="{{ route('daftar_trip.destroy', $daftarTrip->id) }}" method="POST">
                            <a class="inline-block align-middle text-center select-none border font-normal whitespace-no-wrap rounded py-1 px-3 leading-normal no-underline bg-blue-600 text-white hover:bg-blue-600" href="{{ route('daftar_trip.edit', $daftarTrip->id) }}">Edit</a>
                            @csrf
                            @method('DELETE')
