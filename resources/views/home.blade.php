...
<div class="flex flex-wrap justify-center mt-3">
    <div class="w-1/4 pb-6">
        <div class="h-32 flex items-center justify-center">
            <p id="digital-clock" class="text-gray-800 font-bold text-4xl lg:text-6xl sm:text-sm drop-shadow-lg">Jam</p>
        </div>        
    </div>        
    <div class="w-2/4 pb-6 pr-3 pl-3">
        <div class="rounded-lg h-32 flex" style="background-image: url('/image/dashboard.jpg');">
            <p class="text-white text-xl font-bold mt-12 ml-10">Dashboard Admin</p>
        </div>
    </div>    
    <div class="w-1/4 pb-6">
        <div class="h-32 flex flex-col items-center justify-center">
            <p id="date" class="text-gray-800 text-xl">Tanggal</p>
            <p id="day" class="text-gray-800 text-lg font-bold mt-2">Hari</p>
        </div>
    </div>
</div>

<div class="flex mb-12 ml-3 mr-6">
   <div class="w-full lg:w-9/12 bg-blue-200 shadow-blue2 border border-blue-300 p-10 rounded-lg flex items-center justify-center lg:mb-0">
        <canvas id="myChart" class="w-full h-full"></canvas>
    </div>                  
    <div class="w-full lg:w-3/12 flex flex-col lg:ml-4">
        <div class="flex flex-col bg-red-500 rounded-full mb-4 shadow-red">
            <div class="p-4">
                <span class="text-white font-bold">Jumlah Anggota: </span>
                <span class="text-white"><b>{{ $totalAnggota }}</b> Orang</span>
            </div>
        </div>        
        <div class="flex flex-col bg-blue-300 rounded-full shadow-blue mb-4">
            <div class="p-4">
                <span class="text-white font-bold">Jumlah OTA:</span>
                <span class="text-white"><b>{{ $totaldaftarOta }}</b> Orang</span>
            </div>
        </div>
        <div class="flex flex-col bg-gray-500 shadow-gray rounded-full">
            <div class="p-4">
                <span class="text-white font-bold">Jumlah Pesantren:</span>
                <span class="text-white"><b>{{ $totalpondok }}</b>&nbsp; Pondok</span>
            </div>
        </div>
        <div class="flex flex-col mt-4 bg-green-400 border border-green-500 shadow-green rounded-full">
            <div class="p-4">
                <span class="text-white font-bold">Total Beras Masuk:</span>
                <span class="text-white"><b>{{ $totalDanaMasuk }}</b>&nbsp; Sak</span>
            </div>
        </div>
        <div class="flex flex-col mt-4 bg-yellow-300 border border-yellow-400 shadow-yellow rounded-full">
            <div class="p-4">
                <span class="text-white font-bold">Total Beras Keluar:</span>
                <span class="text-white"><b>{{ $totalDanaKeluar }}</b>&nbsp; Sak</span>
            </div>
        </div>
        <div class="flex flex-col pb-3 mt-4 bg-purple-300 border border-purple-400 shadow-purple rounded-lg">
...
var ctx = document.getElementById('myChart').getContext('2d');
    ...
    var chart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                label: 'Grafic Data',
                data: [{{ $totalDanaMasuk }}, {{ $totalDanaKeluar }}, {{ $totalpondok }}, {{ $totaldaftarOta }}, {{ $totalAnggota }}],
    ...
    var data = {
        ...
        datasets: [{
            label: '(satuan Sak)',
            data: [{{ $totalDanaMasuk }}, {{ $totalDanaKeluar }}, {{ $totalDanaMasuk - $totalDanaKeluar }}],
